...
4. **比价（NEGO）**：点 A 列选中几行 → `Ctrl+B` 打开比价面板（参照汇总表 NEGO sheet）——
   各家单价并排、最低价高亮、按**下单数量**算各家总额与最优拆分总额；也可直接输入 P/N 回车，
   面板用「数量档：1 / 3 / 6 / 9 / 12」列出总表里该件的有效数量档，点档位即带出对应行；
   「复制为表格」把结果以表格放进剪贴板，可直接粘贴回 Excel 的 NEGO sheet
